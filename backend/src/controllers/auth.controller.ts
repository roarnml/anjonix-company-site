...
/**
 * Helper to send signup verification emails.
 * Adapts to a single-object sendEmail signature: sendEmail({ to, subject, text, html })
 */
async function sendSignupEmail(to: string, verificationLink: string) {
  const subject = "Verify your account";
  const text = `Click the link to verify: ${verificationLink}`;
  const html = `<p>Welcome! Click <a href="${verificationLink}">here</a> to verify your email.</p>`;
...
export async function signup(req: Request, res: Response) {
  try {
    ...
    if (!email || !password || !category || !role) {
      return res
        .status(400)
        .json({ error: "Missing required fields: email, password, category, role" });
    }

    const cat = (category as string).toLowerCase();
    const normalizedRole = normalizeRole(role); // expects values like 'student', 'instructor', etc.
    ...
    if (cat === "online" && !["student", "instructor"].includes(normalizedRole)) {
      return res
        .status(403)
        .json({ error: "Online signup allowed only for student/instructor" });
    }
    if (cat !== "online" && !["management", "instructor", "student"].includes(normalizedRole)) {
      return res
        .status(403)
        .json({ error: "Invalid role for enterprise/institution" });
    }
    ...
    if (cat !== "online") {
      if (organizationId) {
        // Attach to existing org
        org = await prisma.organization.findUnique({ where: { id: Number(organizationId) } });
        if (!org) return res.status(400).json({ error: "Organization not found" });
    ...
    // normalizedRole should match the enum names (superuser | visitor | vendor | admin | instructor | student | management)
    const roleValue = (normalizedRole ? (normalizedRole as unknown as UserRole) : undefined) as
      | UserRole
      | undefined;
    ...
    const user = await prisma.user.create({
      data: {
        email,
        passwordHash,
        category: cat as any, // Prisma enum Category, we assume input is valid after checks
...
export async function forgotPassword(req: Request, res: Response) {
  try {
    ...
    // Use object-style sendEmail to match utility's expected argument
    await sendEmail({
      to: user.email,
