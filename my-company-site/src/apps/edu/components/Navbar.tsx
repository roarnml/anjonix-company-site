...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white shadow relative z-50 w-screen">
      {/* Navbar Header */}
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        <div className="text-2xl font-bold"><a href="/" className="text-gray-500 font-bold ">Anjonix</a></div>
          ...
          {menuData.map((menu) => (
            <li key={menu.name} className="relative">
              <button
                onClick={() => toggleMenu(menu.name)}
                className={`cursor-pointer focus:outline-none ${
                  activeMenu === menu.name ? "text-indigo-600" : ""
                }`}
              >
                {menu.name}
              </button>
          ...
          {menuData.map((menu) => (
            <div key={menu.name}>
              ...
              {activeMenu === menu.name && (
                <div className="pl-4 py-2 space-y-4 border-l-2 border-indigo-900">
