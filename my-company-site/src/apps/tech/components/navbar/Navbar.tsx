...
export default function Navbar({ variant = "tech" }: NavbarProps) {
  ...
  const renderDropdownColumns = (sections: SubMenuItem[]) => {
    return (
      <div className="fixed left-0 top-[60px] mt-2 w-full bg-white shadow-lg border-t z-50 text-black">
        <div className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-3 gap-5">
  ...
  // ---------- RENDER ----------
  return (
    <nav className="bg-gradient-to-br from-blue-300 to-white shadow-md sticky top-0 z-50 w-full text-black">
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden bg-white px-4 pb-4 border-t h-screen overflow-y-auto">
          {renderMobileLinks()}
        </div>
      )}
    </nav>
